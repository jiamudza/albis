  ...
  const handleSubmit = () => {
    axios.post('/api/submitExam', { ...answers, ...userData })
      .then(response => {
        ...
        localStorage.removeItem('calonSiswa');
        router.push('https://smpitalbanna.sch.id');
      })
      .catch(error => {
    ...
    <div className="min-h-screen bg-[#eef1f5] py-10 relative">
      <div className="max-w-4xl mx-auto px-6 space-y-10">
        ...
        <div className="p-8 rounded-2xl bg-[#eef1f5]
          ...
          {exam.instructions && <p className="mt-3 text-gray-600">{exam.instructions}</p>}
        </div>
            ...
            {section.questions.map(q => (
              ...
              >
                ...
                {q.type === 'multiple_choice' &&
                  q.options?.map(opt => (
                    <label key={opt} className="flex items-center gap-3 p-3 rounded-xl
                      bg-[#eef1f5] shadow-[inset_2px_2px_5px_#cfd4da,inset_-2px_-2px_5px_#ffffff] cursor-pointer"
                    >
                ...
                {q.type === 'checkbox' &&
                  q.options?.map(opt => (
                    <label key={opt} className="flex items-center gap-3 p-3 rounded-xl
                      bg-[#eef1f5] shadow-[inset_2px_2px_5px_#cfd4da,inset_-2px_-2px_5px_#ffffff] cursor-pointer"
                    >
                      <input type="checkbox" checked={(answers[q.id] || []).includes(opt)} onChange={() => toggleCheckbox(q.id, opt)} />
                      {opt}
                    </label>
                  ))}
                ...
                {q.type === 'true_false' &&
                  ['Benar', 'Salah'].map(val => (
                    <label key={val} className="flex items-center gap-3 p-3 rounded-xl
                      bg-[#eef1f5] shadow-[inset_2px_2px_5px_#cfd4da,inset_-2px_-2px_5px_#ffffff]"
                    >
                      <input type="radio" name={`q-${q.id}`} checked={answers[q.id] === val} onChange={() => setAnswer(q.id, val)} />
                      {val}
                    </label>
                  ))}
                ...
                {q.type === 'short_answer' && (
                  <input type="text" value={answers[q.id] || ''} onChange={e => setAnswer(q.id, e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-[#eef1f5]
                      shadow-[inset_3px_3px_6px_#cfd4da,inset_-3px_-3px_6px_#ffffff] focus:outline-none"
                  />
                )}
                  ...
                  <div className="space-y-3">
                    {Object.entries(q.pairs).map(([key, val]) => (
                      <div key={key} className="flex items-center gap-3">
                        <span className="w-32 text-gray-700">{key}</span>
                        <select value={answers[q.id]?.[key] || ''} onChange={e => setAnswer(q.id, { ...(answers[q.id] || {}), [key]: e.target.value })}
                          className="flex-1 px-4 py-2 rounded-xl bg-[#eef1f5]
                            shadow-[inset_3px_3px_6px_#cfd4da,inset_-3px_-3px_6px_#ffffff] focus:outline-none"
                        >
                          <option value="">Pilih</option>
                          {Object.values(q.pairs).map(v => <option key={v} value={v}>{v}</option>)}
                        </select>
                      </div>
                    ))}
                  </div>
        ...
        <div className="flex justify-end">
          <button
            ...
            className="px-6 py-3 rounded-2xl bg-[#eef1f5]
              shadow-[6px_6px_12px_#cfd4da,-6px_-6px_12px_#ffffff]
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-96 shadow-lg space-y-4">
            ...
            <p className="text-gray-600">Apakah Anda yakin ingin mengirim jawaban?</p>
